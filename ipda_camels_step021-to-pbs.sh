...
Description: Write and submit PBS jobs for Step 021 of the
CAMeLS pipeline (CRISPR Analysis Method for Library Screens). 

Usage: bash ipda_camels_step021-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline in-house: -m 80 -c 8 -w "01:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            /path/from/working/dir/to/raw/reads/stem*1.f*
                            of R1 file in individual line and no full stops.
                            Warning: The script assumes R1 is given for output naming purposes,
                            but either R1 or R2 could be given as input.
                            Tipically R1 is used for this analysis.
                            ...
                            Col4:
                            partial-guide-size-INT
                            E.g. 21 for a library with 23nt guides - purpose is to find matchs that do not cover full guide
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_camels_step021-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_camels021-to-pbs_${thislogdate}.txt
...
out_path_step021_BBDuk="camels021_plasmid-rep_BBDuk_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step021_BBDuk}
...
date
echo "## Executing bash ipda_camels_step021-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step021_BBDuk}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#${module_bbduk}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run BBDuk Perfect full length matches at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}"; libraryfasta=`grep "${path_file}" ${input} | cut -f2 | sort | uniq`; full_length=`grep "${path_file}" ${input} | cut -f3 | sort | uniq`; echo "${module_bbduk} t=${ncpus} -Xmx${mem}g ref=${libraryfasta} in=${f1} out=${out_path_step021_BBDuk}/perfect-${full_length}-match_non-guides_${file}_R1.fq.gz outm=${out_path_step021_BBDuk}/perfect-${full_length}-match_${file}_R1.fq.gz stats=${out_path_step021_BBDuk}/perfect-${full_length}-match_stats_${file}_R1 hdist=0 k=${full_length}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run BBDuk Partial perfect shorter length matches at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}"; libraryfasta=`grep "${path_file}" ${input} | cut -f2 | sort | uniq`; shorter_length=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; echo "${module_bbduk} t=${ncpus} -Xmx${mem}g ref=${libraryfasta} in=${f1} out=${out_path_step021_BBDuk}/partial-perfect-${shorter_length}-match_non-guides_${file}_R1.fq.gz outm=${out_path_step021_BBDuk}/partial-perfect-${shorter_length}-match_${file}_R1.fq.gz stats=${out_path_step021_BBDuk}/partial-perfect-${shorter_length}-match_stats_${file}_R1 hdist=0 k=${shorter_length}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run BBDuk Partial 2MM shorter length matches at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}"; libraryfasta=`grep "${path_file}" ${input} | cut -f2 | sort | uniq`; shorter_length=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; echo "${module_bbduk} t=${ncpus} -Xmx${mem}g ref=${libraryfasta} in=${f1} out=${out_path_step021_BBDuk}/partial-2MM-${shorter_length}-match_non-guides_${file}_R1.fq.gz outm=${out_path_step021_BBDuk}/partial-2MM-${shorter_length}-match_${file}_R1.fq.gz stats=${out_path_step021_BBDuk}/partial-2MM-${shorter_length}-match_stats_${file}_R1 hdist=2 k=${shorter_length}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run BBDuk Partial 3MM shorter length matches at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}"; libraryfasta=`grep "${path_file}" ${input} | cut -f2 | sort | uniq`; shorter_length=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; echo "${module_bbduk} t=${ncpus} -Xmx${mem}g ref=${libraryfasta} in=${f1} out=${out_path_step021_BBDuk}/partial-3MM-${shorter_length}-match_non-guides_${file}_R1.fq.gz outm=${out_path_step021_BBDuk}/partial-3MM-${shorter_length}-match_${file}_R1.fq.gz stats=${out_path_step021_BBDuk}/partial-3MM-${shorter_length}-match_stats_${file}_R1 hdist=3 k=${shorter_length}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including CAMeLS step 021 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step021_BBDuk},'"${out_path_step021_BBDuk}"',g' "$logfile"
