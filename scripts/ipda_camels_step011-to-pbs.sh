...
Usage: bash ipda_camels_step011-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline in-house: -m 10 -c 1 -w "02:00:00"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_camels_step011-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_camels011-to-pbs_${thislogdate}.txt
...
out_path_step011_FastQC="camels011_qc-raw_FastQC_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step011_FastQC}
...
date
echo "## Executing bash ipda_camels_step011-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step011_FastQC}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}*1.f*"; f2="${path_file}*2.f*"; echo "fastqc -t ${ncpus} --outdir ${out_path_step011_FastQC} ${f1} ${f2} --memory 10000" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
sed -i 's,${out_path_step011_FastQC},'"${out_path_step011_FastQC}"',g' "$logfile"
