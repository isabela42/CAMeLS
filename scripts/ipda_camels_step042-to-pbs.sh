...
# positives
cut -f1 ${input} | sort | uniq | while read file; do target=`grep "${file}" ${input} | cut -f5 | sort | uniq`; celltype=`grep "${file}" ${input} | cut -f2 | sort | uniq`; positive=`grep "${file}" ${input} | cut -f4 | sort | uniq`; negative=`grep "${file}" ${input} | cut -f3 | sort | uniq`; gtf=`grep "${file}" ${input} | cut -f6 | sort | uniq`; stem=`echo ${file} | rev | cut -d'/' -f1 | rev | cut -d'_' -f2`; ls ${out_path_step042_summary}/${celltype}_${stem}* | while read stats; do cat ${positive} | while read pos; do mainstats=`echo ${stats} | rev | cut -d'/' -f1 | cut -d'.' -f2- | rev`; grep -w "${pos}" ${stats} >> ${out_path_step042_summary}/${mainstats}.positives.txt ; done ; done; done
