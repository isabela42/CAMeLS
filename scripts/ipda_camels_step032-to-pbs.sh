...
cut -f1 ${input} | sort | uniq | while read celltype; do echo "Rscript ${out_path_step032_MAGeCK}/${celltype}_nctrl-all-replicates.report.Rmd" >> ${pbs_stem}_${celltype}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read celltype; do echo "" >> ${pbs_stem}_${celltype}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read celltype; do echo "Rscript ${out_path_step032_MAGeCK}/${celltype}_median-all-replicates.report.Rmd" >> ${pbs_stem}_${celltype}_${thislogdate}.pbs; done
